...
export function IndvBankAccountsEditor({ value, onChange, disabled }: CustomComponentProps) {
  ...
  const config = useMemo<TableConfig>(
    () => ({
      ...
      columns: [
        ...
        { key: 'status', title: 'Status', dataIndex: 'status', render: 'renderStatus', width: 100 },
    ...
    return (
      <div>
        <div className="empty-state" style={{ padding: 24 }}>
          <div className="t-mute fs-12">{tr ? 'Henüz banka hesabı eklenmedi' : 'No bank accounts yet'}</div>
        </div>
      ...
      <div style={{ margin: '-18px -18px 14px' }}>
        <DynamicTable
          ...
          customFunctions={{
            ...
            renderStatus: () => <span className="st active">{t('ib_status_Active')}</span>,
