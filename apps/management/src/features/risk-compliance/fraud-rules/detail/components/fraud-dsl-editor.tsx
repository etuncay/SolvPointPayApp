...
import type { FraudDslScope } from '../../../shared/rule-dsl/variables';
import { FRAUD_SCOPE_VARIABLES } from '../../../shared/rule-dsl/variables';
import { fraudEntityHint } from '../../../shared/rule-dsl/fraud-entities';
import type { DslValidationResult } from '../../../shared/rule-dsl/parser';

export function FraudDslEditor({
  value,
  onChange,
  scope,
  onValidate,
  ...
  const vars = FRAUD_SCOPE_VARIABLES[scope].join(', ');
  ...
  return (
    <div>
      <textarea
        ...
        placeholder={t('frd_dsl_ph')}
      />
      <p className="fs-11 t-mute" style={{ marginTop: 6 }}>
        {t('frd_dsl_entities')}: {fraudEntityHint()} · {t('frd_dsl_example')}
      </p>
      <p className="fs-11 t-mute" style={{ marginTop: 2 }}>
        {t('rs_dsl_vars')}: {vars}
      </p>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 8 }}>
        ...
        {validatedAt && !lastResult && (
          <span className="fs-11 t-mute">{t('frd_validated_prev')}</span>
        )}
      </div>
    </div>
  );
