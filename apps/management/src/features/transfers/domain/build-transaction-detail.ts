...
import { joinTransactionNotes } from './format-transaction-note';
...
function enrichTx(tx: Transaction): Transaction {
  ...
  return {
    ...tx,
    foreignReferenceNo: tx.foreignReferenceNo ?? (tx.type === 'InternationalTransfer' ? `FX-${tx.referenceNo}` : null),
    feeFixed,
    feeVariable,
    fxRate,
    targetAmount,
...
export function buildTransactionDetail(
  ...
  return {
    ...
    notes: sortedNotes,
    notesDisplay: joinTransactionNotes(sortedNotes),
    documents,
    activeBlock,
