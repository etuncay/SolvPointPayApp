...
const TX_OPTION_KEYS: Record<string, string> = {
  WalletToPerson: 'ccm_tx_WalletToPerson',
  InternationalTransfer: 'ccm_tx_InternationalTransfer',
  WalletToBankAccount: 'ccm_tx_WalletToBankAccount',
  WalletTopUp: 'ccm_tx_WalletTopUp',
};

const STATUS_OPTION_KEYS: Record<string, string> = {
  Active: 'ib_status_Active',
  Passive: 'rs_status_passive',
};
...
/** JSON tablo config etiketlerini ccm_* / paylaşımlı i18n key'lerine bağla. */
export function localizeCustomerCampaignsTableConfig(base: TableConfigJson, t: TranslateFn): TableConfigJson {
  return {
    ...base,
    title: t('s_cust_campaign', base.title),
    search: base.search
      ? { ...base.search, placeholder: t('ccm_search_ph', base.search.placeholder ?? '') }
      : base.search,
    ...
    toolbar: base.toolbar
      ? {
          ...
          new: base.toolbar.new
            ? { ...base.toolbar.new, label: t('ccm_new', base.toolbar.new.label) }
            : base.toolbar.new,
