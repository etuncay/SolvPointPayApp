import { z } from 'zod';
import { assertAuthorizedPersonAge } from './age-guard';
...
/** Spec §7 — yetkili kişi zod şeması (banka/cüzdan yok) */
export const authorizedPersonFormSchema = z
  .object({
    ...
    idCountry: z.string().min(1, 'ap2_id_country_required'),
    ...
    serialNo: z.string().min(5, 'ap2_serial_required').max(30),
    ...
    notes: z
      .string()
      .optional()
      .refine((v) => !v || !PII_IN_NOTES.test(v), { message: 'ap2_notes_pii' }),
  ...
  .superRefine((data, ctx) => {
    ...
    const ageErr = assertAuthorizedPersonAge(data.birthDate);
    if (ageErr) {
      ctx.addIssue({ code: 'custom', message: ageErr, path: ['birthDate'] });
    }
    ...
    if (!phones.some((c) => c.verified)) {
      ctx.addIssue({ code: 'custom', message: 'ap2_phone_verify_required', path: ['contacts'] });
    }
    if (!emails.some((c) => c.primary) || !phones.some((c) => c.primary)) {
      ctx.addIssue({ code: 'custom', message: 'ap2_primary_contact_required', path: ['contacts'] });
    }
...
/** Kayıt için Identity belgesi zorunlu */
export function validatePersonDocumentsForSave(
...
export type AuthorizedPersonFormSchema = z.infer<typeof authorizedPersonFormSchema>;
