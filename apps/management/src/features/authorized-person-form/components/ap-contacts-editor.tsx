...
let nextContactId = 9500;

export function APContactsEditor({ value, onChange, disabled }: CustomComponentProps) {
    ...
    <div>
      {rows.map((c, index) => (
        <div key={c.id} style={{ display: 'flex', gap: 8, marginBottom: 8, alignItems: 'center' }}>
          ...
          {c.verified ? (
            <span className="verif ok"><span className="d" />OK</span>
          ) : (
            !disabled && (
          ...
          {c.primary ? (
            ...
            !disabled && (
              <Button type="button" variant="ghost" onClick={() => setPrimary(index)}>
                {tr ? 'Asıl' : 'Primary'}
              </Button>
            )
          )}
