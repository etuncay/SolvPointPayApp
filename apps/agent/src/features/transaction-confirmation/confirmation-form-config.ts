...
import confirmationFormJson from './config/confirmation.form.config.json';
...
const PANEL_KEYS: Record<string, string> = {
  ...
  agents: 'ag_cf_panel_agent',
  security: 'ag_cf_panel_security',
};
...
/** İşlem onay/detay — DynamicForm JSON. */
export function buildConfirmationFormConfig(t?: TranslateFn): FormConfig {
  const base = confirmationFormJson as FormConfig;
  if (!t) return base;
  return {
    ...base,
